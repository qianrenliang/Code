...
class ProcessData:
    ...
    def link_plot(self, lastlinkid, linkcube):
        ...
        for day, dayvalues in linkcube.items():
            for hour, hourvalues in dayvalues.items():
                ...
                for delta, deltavalues in hourvalues.items():
                    Y_1.append(deltavalues['traveltime'])
        # pyplot.plot(X,Y)
        # pyplot.xlabel("DAY")
        ...
        ax.plot(Y_1, label='original data')
        ax.plot(Y_2, label='cleaned data')
        pyplot.legend()
        pyplot.xlabel('DAY')
        pyplot.ylabel('Travel Time')
        pyplot.title("linkid:%s" % lastlinkid)
        pyplot.savefig('./link_plot/%s.jpg' % lastlinkid, dpi=300)
        # pyplot.show()


    def explore_weekdayvalues(self, lastlinkid, linkcube):
        """按周几画出数据量"""
        DAY0,DAY1,DAY2,DAY3,DAY4,DAY5,DAY6=0,0,0,0,0,0,0
        for day,dayvalues in linkcube.items():
            for hour,hourvalues in dayvalues.items():
                for delta,deltavalues in hourvalues.items():
                    if deltavalues['weekdayid'] == 0:
                        DAY0 += 1
                    elif deltavalues['weekdayid'] == 1:
                        DAY1 += 1
                    elif deltavalues['weekdayid'] == 2:
                        DAY2 += 1
                    elif deltavalues['weekdayid'] == 3:
                        DAY3 += 1
                    elif deltavalues['weekdayid'] == 4:
                        DAY4 += 1
                    elif deltavalues['weekdayid'] == 5:
                        DAY5 += 1
                    else:  DAY6 += 1
        Y = [DAY0,DAY1,DAY2,DAY3,DAY4,DAY5,DAY6]
        X = range(7)
        pyplot.plot(X,Y)
        pyplot.xticks(X,('Sunday','Monday','Tuesday','Wensday','Thursday','Friday','Saturday'))
        pyplot.title("linkid:%s"%lastlinkid)
        pyplot.show()
            ...
            for hourid, houridvalue in dayvalue.items():
                ...
                for deltaid, deltaidvalue in houridvalue.items():
                    ...
                    for lastmonthday in lastmonthdays:
                        if lastmonthday not in linkcube or hourid not in linkcube[lastmonthday] or deltaid not in \
                                linkcube[lastmonthday][hourid]:
                            lastmonth_values.append(0)
                        else:
                            lastmonth_values.append(linkcube[lastmonthday][hourid][deltaid]['traveltime'])

                    # 抛弃timeseries  只考虑当天的前一小时历史数据
                    Today_series = []
                    ...
                    # 加入组合标识信息，不用于模型训练
                    sample += [deltaidvalue['linkid'], deltaidvalue['dayid'], deltaidvalue['hourid'],
                               deltaidvalue['deltaid']]
        ...
        for line in infile:
            ...
            if linkid != lastlinkid:  # 新的Linkid
                if not linkcube:  # 第1个linkid
                    ...
                    linkcube[dayid][hourid][deltaid] = {'weekdayid': weekdayid, 'isholiday': isholiday,
                                                        'length': link_length, 'width': link_width,
                                                        'traveltime': travel_time, 'linkid': linkid, 'dayid': dayid,
                                                        'hourid': hourid, 'deltaid': deltaid}
                    continue

                # self.explore_weekdayvalues(lastlinkid,linkcube)

                # linkcube = self.checkoutliers(lastlinkid, linkcube)
                # self.print_out(lastlinkid, linkcube)

                self.link_plot(lastlinkid, linkcube)
            ...
            linkcube[dayid][hourid][deltaid] = {'weekdayid': weekdayid, 'isholiday': isholiday, 'length': link_length,
                                                'width': link_width,
                                                'traveltime': travel_time, 'linkid': linkid, 'dayid': dayid,
                                                'hourid': hourid, 'deltaid': deltaid}
    ...
    def save_to_file(self, keyid, pre, filename):
        ...
        res.to_csv(filepath + '\\' + '%s.txt' % filename, header=None, index=None, sep='#',
                   columns=['linkid', 'dateid', 'hourid', 'deltaid', 'value'])
        return 0
    ...
    def train(self, get_submit=False):
        ...
        for linkid, linkid_vaules in self.samples.items():
            ...
            reg, predict_train, predict_eval, predict_test = self.xgb_process(training_x, training_y, evaling_x,
                                                                              evaling_y, testing_x, 8, 500, 0.8)
    ...
    def xgb_online_process(self, localX, localY, onlineX, onlineY, evalX, evalY, testX, depth, iterations,
                           colsample_bytree):
        ...
        tree_booster_online = xgb.train(params=paras, dtrain=training_online_set, num_boost_round=best_iteration,
                                        obj=maelikeobj, feval=eval_log_mape)

        predict_local_train = tree_booster_local.predict(training_local_set, output_margin=False,
