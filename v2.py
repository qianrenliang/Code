...
class ProcessData:
    ...
    def print_out(self, lastlinkid, linkcube):
        ...
        for day, dayvalue in linkcube.items():
            ...
            lastmonthdays = sorted(lastmonthdays) # 4个dayid
            historydays = sorted(historydays) # 共9个dayid
                ...
                for deltaid, deltaidvalue in houridvalue.items():
                    ...
                    for historyday in historydays:
                        for delta_id in range(self.secs,-1,-1):
                            # self.secs = 30   >> [30.....,3,2,1,0]  间隔是2mins, 31个数据
                            # [8:02~8:04) 取 [6:02~6:04)~[7:02~7:04)一小时的数据
                    ...
                    hour_values = []  # 当天前1小时数据  如7/23 8:00~8:02数据条  抽取 7/23 [6:00:6:02] ~ [7:00:7:03] 数据
            ...
            for hour, hourvalues in level_vaules.items():
                ...
                for delta, deltavaules in hourvalues.items():
                    for day,dayvalues in deltavaules.items():
                        ...
                        # 线上提交：2016-04-13 ~ 2016-05-31数据进行模型的训练，最后5天做模型eval_local
